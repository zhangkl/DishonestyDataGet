...
public class HttpUtil {
    ...
    public Object doGet(String url, Map params) throws InterruptedException, IOException, NetWorkException {
        ...
        RequestConfig requestConfig = RequestConfig.custom().setConnectTimeout(1000 * 60).setConnectionRequestTimeout(1000 * 60)
        ...
        try {
            ...
            if (httpResponse.getStatusLine().getStatusCode() == 200) {
                ...
                Object result;
                if (url.contains("http://shixin.court.gov.cn/image.jsp?date=")) {
                    if (entity.getContentType().getValue().startsWith("image")) {
                        result = EntityUtils.toByteArray(entity);
                    } else {
                        logger.info(EntityUtils.toString(entity, getcharset(httpResponse)));
                        logger.info(cookies);
                        result = null;
                    }
                } else if (entity.getContentType().getValue().startsWith("image")) {
                    result = EntityUtils.toByteArray(entity);
                } else {
